/**
 * Improved Google Apps Script for NSS Website Blog Post Integration
...
/**
 * Enhanced form response parsing with validation
 */
function parseFormResponsesWithValidation(responses) {
  ...
  const eventData = {
    ...
    team: parseCommaSeparated(getResponseValue(11, 'Team Members', '')),
    tags: parseCommaSeparated(getResponseValue(12, 'Tags', '')),
    organizer: getResponseValue(13, 'Organizer', 'NSS IIT BBS'),
    contact: getResponseValue(14, 'Contact Email', 'nss@iitbbs.ac.in')
  };
...
/**
 * Add event to GitHub with enhanced error handling
 */
...
/**
 * Parse current file content with error handling
 */
function parseCurrentContent(content) {
  try {
    // Try to extract the array from module.exports
    const arrayMatch = content.match(/module\.exports\s*=\s*(\[[\s\S]*\]);?/);
    if (arrayMatch) {
      return JSON.parse(arrayMatch[1]);
    }

    // Try alternative patterns
    const altMatch = content.match(/=\s*(\[[\s\S]*\])[\s;]*$/);
    if (altMatch) {
...
function uploadToGitHub(content) {
  ...
  const payload = {
    message: `Add new NSS event via form submission - ${new Date().toISOString()}`,
...
View the website: [Your website URL]
...
/**
 * Test function with comprehensive checks
 */
function testIntegration() {
  ...
  try {
    ...
    if (GITHUB_REPO === 'your_username/NSS-website') {
      throw new Error('GitHub repository not configured');
    }
    console.log('✅ Configuration looks good');

    // Test 2: GitHub API access
    console.log('Test 2: Testing GitHub API access...');
    getCurrentFileContent();
    console.log('✅ GitHub API access successful');

    // Test 3: Create test event
    console.log('Test 3: Creating test event...');
    const testEvent = {
      title: `Integration Test - ${new Date().toISOString()}`,
      date: new Date().toISOString().split('T')[0],
      summary: 'This is a test event created by the integration system to verify functionality.',
      content: 'This test event can be safely deleted. It confirms that the Google Form integration is working correctly and can successfully create blog posts on the NSS website.',
      ...
      team: ['Integration Test Coordinator'],
      tags: ['test', 'integration', 'automated'],
      organizer: 'NSS IIT BBS - Integration Test',
    ...
    if (result.success) {
      ...
      console.log('Test event has been added to the repository.');
      console.log('Check your website to see if it appears in the blog section.');
      sendSuccessNotification(testEvent);
      return true;
    } else {
      throw new Error(result.error);
    }
...
function debugFormSubmission() {
  ...
  const mockFormEvent = {
    values: [
      ...
      'Debug Test Event',        // Title
      '2024-12-25',             // Date
      'Testing form submission', // Summary
      'This is a debug test to verify form processing works correctly.', // Content
      '/api/placeholder/400/300', // Image
      'Debug test image',        // Image Alt
      'Community Service',       // Type
      '25',                      // Participants
      'IIT Bhubaneswar',        // Location
      '2 hours',                // Duration
      'Debug Team Lead, Test Coordinator', // Team
      'debug, test, form',       // Tags
      'NSS IIT BBS Debug',      // Organizer
      'debug@iitbbs.ac.in'      // Contact
    ]
  };
